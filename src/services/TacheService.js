...
async createSousTache(data) {
  try {
    const response = await fetch("http://localhost:5001/api/tache/create_sous_tache", {
      method: "POST",
...
create_comment: async (comments_data) => {
  ...
  get_task_comment: async (ref_tache) => {
    try {
      const response = await fetch(`${API_URL}/commentaires/get_c_tache/${ref_tache}`);
      if (!response.ok) {
    ...
    try {
      const response = await fetch(`${API_URL}/commentaires/get_c_sous_tache/${ref_sous_tache}`);
      if (!response.ok) {
